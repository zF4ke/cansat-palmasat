...
class DFRobot_BMP388(object):
  ...
  def parse_calib_data(self,calib):
    ...
    calibTempA = (calib[6]<<8|calib[5])
    calibTempB = self.uint16_int(calibTempA)
    self.par_p1 = ((calibTempB)-16384)/temp_var
    ...
    calibTempA = (calib[8]<<8|calib[7])
    calibTempB = self.uint16_int(calibTempA)
    self.par_p2 = ((calibTempB)-16384)/temp_var
    #print((calibTempA<<8)|calibTempB)
    ...
    temp_var = 281474976710656.0
    #calibTempA = self.uint8_int(calib[18])
    #calibTempB = (calib[17])
    #self.par_p9 = (((calibTempA<<8)|calibTempB))/temp_var
    calibTempA = (calib[18]<<8|calib[17])
    calibTempB = self.uint16_int(calibTempA)
    self.par_p9 = calibTempB/temp_var
  ...
  #set_pwr_ctrl_settings
    ...
    #set oversampling rate - use or mask to leave reserved bits and set variable bits to 1
    # then use & mask to leave reserved bits and assign the bits we want to set
    # osr_T is bits 3-5 osr_P is bits 0-2
    # reccommended settings for drone are osr_P x8 =011 osr_T x1 =000
    osr_reg=0x1c
    osr_reg_data = self.bmp3_get_regs(osr_reg,1)[0]
    osr_data = [osr_reg_data]
    #print("osrdata")
    #print(osr_data)
    #print(osr_data[0])
    osr_reg_data=((osr_reg_data|(0x3f))&(0xc3))
    osr_data = [osr_reg_data]
    #print(osr_data)
    #print(osr_data[0])
    self.bmp3_set_regs(osr_reg,osr_data)
    #check its done
    osr_reg_data = self.bmp3_get_regs(osr_reg,1)[0]
    osr_data = [osr_reg_data]
    #print("osrdata set ")
    ...
    #set iir filter - use or mask to leave reserved bits and set variable bits to 1
    # then use & mask to leave reserved bits and assign the bits we want to set
    # iir filter coef is bits 1-3 (bit 0 and bits 4-7 reserved)
    # recommended settings to match osr for drone are iir filter coef_3 =010
    iir_reg=0x1f
    iir_reg_data = self.bmp3_get_regs(iir_reg,1)[0]
    iir_data = [iir_reg_data]
    #print("iirdata")
    #print(iir_data)
    #print(iir_data[0])
    iir_reg_data=((iir_reg_data|(0x06))&(0xfd))
    iir_data = [iir_reg_data]
    #print(iir_data)
    #print(iir_data[0])
    self.bmp3_set_regs(iir_reg,iir_data)
    #check its done
    iir_reg_data = self.bmp3_get_regs(iir_reg,1)[0]
    iir_data = [iir_reg_data]
    #print("iirdata set ")
    ...
    #set data rate - use or mask to leave reserved bits and set variable bits to 1
    # then use & mask to leave reserved bits and assign the bits we want to set
    # data time for above osr and filter is up to 20ms so set sampling period to 40ms
    # odr_sel is ODR_25 or prescaler 8 =00011 (set bits 0-4 reserved bits 5-7)
    odr_reg=0x1d
    odr_reg_data = self.bmp3_get_regs(odr_reg,1)[0]
    odr_data = [odr_reg_data]
    #print("odrdata")
    #print(odr_data)
    #print(odr_data[0])
    odr_reg_data=((odr_reg_data|(0x1f))&(0xe3))
    odr_data = [odr_reg_data]
    #print(odr_data)
    #print(odr_data[0])
    self.bmp3_set_regs(odr_reg,odr_data)
    #check its done
    odr_reg_data = self.bmp3_get_regs(odr_reg,1)[0]
    odr_data = [odr_reg_data]
    #print("odrdata set ")
  ...
  def bmp3_get_sensor_data(self,sensor_comp):
    ...
    #print("uncomp pressure",uncomp_pressure)
    xlsb = rslt[3]
    lsb = rslt[4] << 8
    msb = rslt[5] << 16
  ...
  def readCalibratedAltitude(self,SeaLevelP,SeaLevelT):
    pressure = self.readPressure()
    return round((1.0 - pow(pressure / SeaLevelP, 0.190269)) * (SeaLevelT+273.15) / 0.0065,2)

  def readSeaLevelP(self, LocalAltitude):
    pressure = self.readPressure()
    abstemperature = self.readTemperature()+273.15
    return round(pressure / pow(1.0 - (LocalAltitude *0.0065/ (abstemperature+LocalAltitude*0.0065)), 5.255724),2)

  def readSeaLevelT(self, LocalAltitude):
    temperature = self.readTemperature()
    return round(temperature + (LocalAltitude *0.0065),2)



  def readAltitude(self):
    pressure = self.readPressure()
    return round((1.0 - pow(pressure / 101325, 0.190269)) * 288.15 / 0.0065,2)
